...
$(document).ready(function () {
    ...
    $('.btn-edit').click(function () {
        ...
        $('#branches').val($(this).data('branches'));
        $('#status').val($(this).data('status'));
        $('#userForm').attr(
            'action',
            '/users/' + $(this).data('id') + '?_method=PUT'
        );
    ...
    function calculateOrder() {
        ...
        // $('#ordSGST').val(sgst.toFixed(2));
        // $('#ordCGST').val(cgst.toFixed(2));
    ...
    $('#invQty,#invCost,#invSGST,#invCGST').on('keyup change', function () {
        calculateInvoice();
    });
